...
from web.config import JOBS_BASE_DIR
...
def test_index():
    ...
    assert "FileOptimizer" in response.text


def test_image_form():
    response = client.get("/image")
    assert response.status_code == 200
    assert "Optimize Image" in response.text or "Оптимизировать изображение" in response.text


def test_archive_form():
    response = client.get("/archive")
    assert response.status_code == 200
    assert "Create Archive" in response.text or "Создать архив" in response.text


def test_image_optimization_success(tmp_path, monkeypatch):
...
def test_archive_creation_success(tmp_path, monkeypatch):
    monkeypatch.setattr("web.config.JOBS_BASE_DIR", tmp_path / "jobs")
...
def test_download_result(tmp_path, monkeypatch):
    monkeypatch.setattr("web.config.JOBS_BASE_DIR", tmp_path / "jobs")
    job_id = "testjob"
    job_dir = tmp_path / "jobs" / job_id
